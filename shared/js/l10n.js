...
(function(window) {
  ...
  function parseResource(href, lang, successCallback, failureCallback) {
    ...
    loadResource(href, function(response) {
      ...
      if (typeof response !== 'string') { // json or binary data
        ...
        // for a given language. All data in the {locale}.properties files
        // should be merged into a json, so we won't overrite gL10nData
        // if there is one.
        if (!gL10nData) {
  ...
  L10nDataStore.prototype._createArrayBufferViews = function() {
    try {
      ...
      this._indexView = new Uint16Array(this._blobData, 8, elements);
